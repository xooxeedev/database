...
-- EMPLOYEE 테이블에서 사원의 이름, 입사일, 근무일수를 조회
-- 컬럼명이 모두 이름, 입사일, 근무일수로 별칭 지정
SELECT EMP_NAME AS 이름, HIRE_DATE 입사일,
...
-- * DISTINCT 주의 사항
--  -> DISTINCT SELECT절 하나당 한 번만 사용 가능 (오류나는게 정상!) 
...
--1. EMPLOYEE 테이블에서 SAL_LEVEL S1인 사원의
-- 이름, 월급, 고용일, 연락처 조회
SELECT EMP_NAME 이름, SALARY 월급, HIRE_DATE 고용일, PHONE 연락처
FROM EMPLOYEE
WHERE SAL_LEVEL = 'S1';

--2. EMPLOYEE 테이블에서 월급이 3,000,000이상인 사원의
-- 이름, 월급, 고용일 조회
...
/*
  ...
  FROM 테이블명
  [WHERE 조건식]
  [ORDER BY 컬러명 | 별칭 | 컬럼순서 정렬 방법(오름, 내림) [NULLS FIRST | LAST] ]
*/

-- EMPLOYEE 테이블에서 
-- 모든 사원의 이름, 급여, 부서코드를 오름차순으로 출력
SELECT EMP_NAME, SALARY, DEPT_CODE
FROM EMPLOYEE
ORDER BY EMP_NAME /*ASC*/; -- 오름 차순이 기본값

-- EMP_NAME 오름차순
SELECT EMP_NAME, SALARY, DEPT_CODE
FROM EMPLOYEE
ORDER BY 1; -- 조회한 컬럼 중 첫 번째를(EMP_NAME)를 기준으로 정렬
...
SELECT EMP_NAME, SALARY * 12 AS 연봉, DEPT_CODE
FROM EMPLOYEE
ORDER BY 연봉 DESC NULLS LAST; -- 별칭 사용 가능
...
SELECT EMP_NAME, SALARY * 12 AS 연봉, DEPT_CODE
FROM EMPLOYEE
WHERE SALARY * 12 /*연봉*/ >= 40000000 -- WHERE절에는 별칭 사용 불가 
ORDER BY 2 DESC NULLS LAST; -- 별칭 사용 가능
